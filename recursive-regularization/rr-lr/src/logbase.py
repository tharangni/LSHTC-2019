...
class LogisticBase(object):
    ...
    def __init__(self, rho, w_n, w_pi, children, mod_cn,
        accelerated=True, max_iter = 2000, tol = 10**-4, 
        beta=0.5, fit_intercept = True, intercept_scaling = 1, 
        feature_scaling = 1, verbosity=0, solver='lbfgs', var_init_type=1):
...
#             raise ValueError("The number of classes has to be greater than one.")
    ...
    def optimize_objective_non_leaf(self, X, y):
        ...
        if self.fit_intercept:
            self.coef_ = self.W[1:,:]
            self.intercept_ = self.W[0,:]*self.intercept_scaling
        else:
            self.coef_ = self.W
            self.intercept_ = 0
